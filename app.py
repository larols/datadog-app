...
@app.route('/')
def index():
    with tracer.trace("request.index") as span:
        span.set_tag("example_tag", "index_page")
        processing_time = random.uniform(0.1, 0.5)
        time.sleep(processing_time)  # Simulate some processing
        return f"Hello, Datadog! Processing time: {processing_time:.2f}s\n"

@app.route('/data')
def data():
    with tracer.trace("request.data") as span:
        span.set_tag("example_tag", "data_page")
